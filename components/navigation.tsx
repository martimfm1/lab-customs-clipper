        ...
        <div className="rounded-2xl border border-white/[0.08] bg-zinc-950/80 shadow-[0_12px_45px_rgba(0,0,0,0.28)] backdrop-blur-xl">
          <div className="flex items-center justify-between px-3 py-2.5 sm:px-4">
            <a
              ...
              className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-white sm:text-xs"
            >
              LAB CUSTOMS CLIPPER
            </a>
            ...
            >
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="rounded-lg px-3.5 py-2 text-xs font-medium text-zinc-400 transition hover:bg-zinc-900 hover:text-white"
                >
                  {link.label}
                </a>
              ))}
              ...
              >
                <nav className="grid gap-1 p-3" aria-label="Navegação móvel">
                  ...
                  <a
                    href="/marcacao"
                    onClick={() => setMenuOpen(false)}
                    className="mt-2 flex h-12 items-center justify-center gap-2 rounded-xl bg-[#a3e635] text-sm font-extrabold text-zinc-950"
                  >
                    <MessageCircle className="h-4 w-4" />
                    Marcar serviço
                  </a>
