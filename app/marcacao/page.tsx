...
export default function BookingMessagePage() {
  ...
  const service = useMemo(() => {
    if (mode === "preset")
      return PRESET_SERVICES.find((s) => s.id === selectedId)?.label ?? "";
        ...
        <div className="hidden lg:flex flex-col justify-between px-16 py-16 border-r border-zinc-800/80">
          ...
          <div className="space-y-6">
            ...
            <h1 className="text-5xl xl:text-6xl font-extrabold tracking-tighter text-white leading-none">
              MARCA O TEU
              <br />
              <span className="text-zinc-400 font-light text-4xl xl:text-5xl">
                serviço.
              </span>
            </h1>

            <p className="text-zinc-400 text-base leading-relaxed max-w-sm text-pretty">
              Na Lista escolhes exatamente o serviço de que precisas e descreves
              a avaria. Em Outro podes escrever um pedido completo com vários
              serviços e vários problemas.
            </p>
          ...
          <div className="lg:hidden mb-8 space-y-6">
            ...
            <div>
              ...
              <h1 className="text-4xl font-extrabold tracking-tighter text-white leading-none">
                MARCA O TEU{" "}
                <span className="text-zinc-400 font-light">serviço.</span>
              </h1>
              <p className="mt-3 text-zinc-400 text-sm leading-relaxed">
                Na Lista escolhes um serviço e explicas a avaria. Em Outro podes
                juntar tudo num único pedido.
              </p>
            </div>
          </div>
          ...
          >
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-wider text-zinc-400">
                <User className="w-3.5 h-3.5 shrink-0 text-[#a3e635]" />O teu
                nome
              </label>
              <input
                type="text"
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-2">
                <label className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-wider text-zinc-400">
                  <Scissors className="w-3.5 h-3.5 shrink-0 text-[#a3e635]" />O
                  que precisas?
                </label>
                ...
                <div className="flex items-center gap-0.5 rounded-lg border border-zinc-800 bg-zinc-950 p-1">
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-[10px] font-mono transition-all duration-200 cursor-pointer ${mode === "preset" ? "bg-[#a3e635] text-zinc-950 font-bold" : "text-zinc-500 hover:text-zinc-300"}`}
                  >
                    Lista
                  </button>
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-md text-[10px] font-mono transition-all duration-200 cursor-pointer ${mode === "custom" ? "bg-[#a3e635] text-zinc-950 font-bold" : "text-zinc-500 hover:text-zinc-300"}`}
                  >
                    ...
                    {PRESET_SERVICES.map((s) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left rounded-xl border px-4 py-3.5 transition-all duration-200 cursor-pointer active:scale-[0.99] ${isSelected ? "border-[#a3e635]/50 bg-[#a3e635]/10 shadow-[0_0_15px_rgba(163,230,53,0.1)]" : "border-zinc-800/80 bg-zinc-950/50 hover:border-zinc-700 hover:bg-zinc-900"}`}
                        >
                          <div className="flex items-center justify-between gap-3">
                            <div className="min-w-0">
                              <p
                                className={`text-sm font-semibold truncate transition-colors ${isSelected ? "text-[#a3e635]" : "text-zinc-200"}`}
                              >
                                {s.label}
                              </p>
                              <p className="text-[10px] inline text-zinc-500 mt-0.5 truncate">
                                {s.description}
                              </p>
                            </div>
                            <div
                              className={`w-4 h-4 rounded-full border shrink-0 transition-all flex items-center justify-center ${isSelected ? "border-[#a3e635] bg-[#a3e635]" : "border-zinc-700 bg-zinc-900"}`}
                            >
                              {isSelected && (
                                <svg
                                  viewBox="0 0 16 16"
                  ...
                  >
                    ...
                    <div className="mt-1 flex items-center justify-between px-1">
                      <p className="text-[10px] font-mono text-zinc-600">
                        Junta vários serviços, máquinas e problemas no mesmo
                        pedido.
                      </p>
                      <span className="text-[10px] font-mono text-zinc-700">
                        {customService.length}/1200
                      </span>
                    </div>
                  </motion.div>
                )}
                ...
                {mode === "preset" && selectedId && (
                  ...
                  >
                    <div className="mt-3 space-y-2">
                      ...
                      <textarea
                        placeholder={
                          selectedId === "preventiva"
                            ? "Ex: Quero fazer a manutenção porque a máquina está com pouco desempenho..."
                            : selectedId === "corretiva"
                              ? "Ex: A máquina liga, mas perde força ao cortar e aquece bastante..."
                              : "Ex: Quero perceber como funciona o plano e o que está incluído..."
                        }
                        value={problem}
                      ...
                      <div className="flex items-center justify-between px-1">
                        <p className="text-[10px] font-mono text-zinc-600">
                          Quanto mais específico, melhor conseguimos perceber o
                          que precisas.
                        </p>
                        <span className="text-[10px] font-mono text-zinc-700">
                          {problem.length}/800
            ...
            <div className="space-y-2">
              ...
              <div className="relative min-h-[96px] rounded-xl border border-zinc-800 bg-zinc-950/80 p-4">
                {message ? (
                  <>
                    <p className="text-xs font-mono text-zinc-300 leading-relaxed pr-8 whitespace-pre-line">
                      {message}
                    </p>
                    <button
                      type="button"
                  ...
                  <p className="text-xs font-mono text-zinc-600 italic">
                    A mensagem irá aparecer aqui à medida que preenches os
                    dados...
                  </p>
                )}
              </div>
            ...
            {!isReady && (
              <p className="text-center text-[11px] font-mono text-zinc-600">
                Preenche o teu nome e indica o serviço ou pedido que precisas
              </p>
            )}
            {isReady && (
              <p className="text-center text-[11px] text-zinc-600">
                Abrimos o WhatsApp com esta mensagem já preparada. Só tens de
                confirmar o envio.
              </p>
            )}
          </motion.div>
